...
@section('content')
    <div class="container mx-auto px-4">
        ...
        <div class="bg-white p-4 shadow rounded-lg mb-6">
            <div class="mb-4 flex justify-end">
                <a href="{{ route('rekap.export-pdf', ['tahun' => request('tahun')]) }}"
            ...
            @foreach ($roadmap as $rm)
                <h2 class="text-2xl font-bold text-gray-700 mt-8 mb-4">Grup Riset : {{ $rm->jenis_roadmap }}</h2>

                @if (isset($penelitiansPerRoadmap[$rm->id]) && $penelitiansPerRoadmap[$rm->id]->count())
                    <table class="w-full border-collapse border border-gray-300 mb-6">
                        ...
                        <tbody>
                            @foreach ($penelitiansPerRoadmap[$rm->id] as $item)
                    ...
                    {{-- Paginasi --}}
                    <div class="mb-8">
                        {{ $penelitiansPerRoadmap[$rm->id]->appends(request()->except('page'))->links() }}
                    </div>
                @else
                    <p class="text-gray-500 italic">Tidak ada data penelitian untuk roadmap ini.</p>
                @endif
            @endforeach
        </div>
        {{-- Statistik Diagram --}}
    <div class="bg-gray-900 text-white p-6 shadow-lg rounded-lg mb-10 max-w-5xl mx-auto">
        <h2 class="text-2xl font-bold mb-6 text-center">
            Statistik Penelitian per Roadmap @if(request('tahun')) ({{ request('tahun') }}) @endif
        </h2>
    ...
    new Chart(document.getElementById('doughnutChart').getContext('2d'), {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: ctx => `${ctx.label}: ${ctx.raw} penelitian`
                    }
                }
            }
        }
