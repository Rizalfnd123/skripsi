...
@section('content')
    <div class="container mx-auto px-4">
        {{-- Judul --}}
        <h1 class="text-3xl font-bold text-gray-800 mb-6 text-center">Luaran Buku ber-ISBN Penelitian</h1>

        {{-- Tombol Tambah + Filter Tahun --}}
        <div class="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
            {{-- Tombol Tambah --}}
            <a href="{{ route('luaran.penelitian.buku.create.dosen') }}"
                class="bg-blue-500 text-white px-4 py-2 rounded shadow hover:bg-blue-600 transition">
                + Tambah Buku
            </a>

            {{-- Form Filter Tahun --}}
            <form method="GET" action="{{ route('luaran.penelitian.buku.dosen') }}">
        ...
        <div class="bg-white p-4 shadow rounded-lg mb-6">
            ...
            @include('dosen.penelitian.luaranBuku._table', ['luarans' => $luaranRoadmap1])

            {{-- ========== ROADMAP 2 ========== --}}
            <h2 class="text-xl font-semibold mt-6 mb-2">Pembelajaran Berbantuan TIK</h2>
            @include('dosen.penelitian.luaranBuku._table', ['luarans' => $luaranRoadmap2])

            {{-- ========== ROADMAP 3 ========== --}}
            <h2 class="text-xl font-semibold mt-6 mb-2">Informatika Terapan</h2>
            @include('dosen.penelitian.luaranBuku._table', ['luarans' => $luaranRoadmap3])
        </div>
    </div>
