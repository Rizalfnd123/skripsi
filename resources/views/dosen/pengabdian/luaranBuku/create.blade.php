...
@section('content')
    <div class="container mx-auto px-4">
        <h1 class="text-3xl font-bold text-gray-800 mb-6 text-center">Tambah Buku ber-ISBN</h1>

        {{-- Form Create Jurnal --}}
        <form action="{{ route('luaran.pengabdian.buku.store.dosen') }}" method="POST" class="bg-white p-6 shadow rounded-lg">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="text" name="isbn" id="isbn" class="w-full px-4 py-2 border rounded-lg">
            </div>
            ...
            <div class="mb-4">
                <label for="penerbit" class="block text-sm font-semibold text-gray-700">Penerbit</label>
                <input type="text" name="penerbit" id="penerbit" class="w-full px-4 py-2 border rounded-lg" required>
            </div>

            {{-- Tahun --}}
            <div class="mb-4">
                <label for="tahun" class="block text-sm font-semibold text-gray-700">Tahun Terbit</label>
                <input type="number" name="tahun" id="tahun" class="w-full px-4 py-2 border rounded-lg" required>
            </div>

            {{-- Link --}}
            <div class="mb-4">
                <label for="link" class="block text-sm font-semibold text-gray-700">Link Buku</label>
